...
namespace cagd
{
    ...
    GLdouble CyclicCurve3::CalculateNormalizingCoefficient (GLuint n)
    {
        if(!n)
        {
            return 1.0;
    ...
    GLvoid CyclicCurve3::CalculateBinomialCoefficients (GLuint m, TriangularMatrix<GLdouble> &bc)
    {
        bc.ResizeRows(m + 1);
        bc(0, 0) = 1.0;
    ...
    CyclicCurve3::CyclicCurve3 (GLuint n, GLenum data_usage_flag):
        LinearCombination3 (0.0, TWO PI, 2 ∗ n + 1, data usage flag ),
        _n(n),
        _c_n(CalculateNormalizingCoefficient (n)),
        _lambda_n (TWO PI / (2 ∗ n + 1))
    {
        _CalculateBinomialCoefficients (2 ∗ _n , _bc);
    }

     // C2n =nC2n,i (u) = cn (1 + cos (u − iλn))n : u ∈ [0, 2π]o2n i=0
