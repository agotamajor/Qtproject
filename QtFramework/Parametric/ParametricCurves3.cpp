...
GenericCurve3* ParametricCurve3::GenerateImage(GLuint div_point_count, GLenum usage_flag, GLuint dernr) const
{
    ...
    {
        if(dernr==0 && order==1){
            (*result)(order, 0) = DCoordinate3(0, 0, 0);
            (*result)(order, div_point_count - 1) =  DCoordinate3(0, 0, 0);
        }else{
    ...
    for (GLuint i = 1; i < div_point_count - 1; i++)
    {
        ...
        {
            if(dernr==0 && order==1){
                (*result)(order, i) = DCoordinate3(0, 0, 0);
            }else{
