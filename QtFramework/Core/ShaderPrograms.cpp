...
void ShaderProgram::transform()
{
    ...
    {   float my_specular_intensity = 0.98;
        ...
        float my_colors [12] = {0.4 , 0.4 , 0.8 , 1.0 , 0.2 , 0.2 , 0.4 , 1.0 , 0.1 , 0.1 , 0.1 , 1.0};
        GLint location = -1;
        location = glGetUniformLocation (_program, "specular_intensity");

    if ( location > 0)
        glUniform1f ( location, my_specular_intensity );

    location = glGetUniformLocation (_program, "specular_color");
    if ( location > 0) glUniform4fv (location, 1, my_specular_color);

    location = glGetUniformLocation (_program, "threshold");
    if ( location > 0) glUniform1fv (location, 2, my_threshold );
    location = glGetUniformLocation (_program, "colors");
    if ( location > 0) glUniform4fv (location, 3, my_colors );
