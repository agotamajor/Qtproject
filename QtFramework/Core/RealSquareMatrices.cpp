...
RealSquareMatrix::RealSquareMatrix(const RealSquareMatrix& m):
    Matrix<GLdouble>(m), _lu_decomposition_is_done(m._lu_decomposition_is_done), _row_permutation(m._row_permutation)
{
...
{
    //return row_count == Matrix<GLdouble>::_column_count;
...
{
    //return row_count == Matrix<GLdouble>::_row_count;
    return Matrix<GLdouble>::ResizeRows(row_count) && Matrix<GLdouble>::ResizeColumns(row_count);
}
