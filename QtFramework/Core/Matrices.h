...
//        // get copy of an element
    ...
    Matrix<T>::Matrix (GLuint row_count, GLuint column_count):
        _row_count(row_count), _column_count(column_count), _data(_row_count, std::vector<T>(_column_count))
    {
    ...
    {
        if(index<0 || index>=_row_count || row.GetColumnCount()!=_column_count){
            return GL_FALSE;
        }
    ...
    GLboolean Matrix<T>::SetColumn(GLuint index, const ColumnMatrix<T>& column)
    {
        ...
        for(GLuint i = 0; i < column.GetRowCount(); i++)
        {
    ...
    std::istream& operator >>(std::istream& lhs, TriangularMatrix<T>& rhs)
    {
        ...
        GLuint current_column_count = 1;

        for (typename std::vector< std::vector<T> >::const_iterator row = rhs._data.begin();
             row != rhs._data.end(); ++row, ++current_column_count)
        {
            row->resize(current_column_count);
